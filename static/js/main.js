...
document
  ...
  .addEventListener("submit", async function (event) {
    ...
    try {
      ...
      // 테이블 데이터에 값 넣기
      tbody.innerHTML = `
      <tr id="${response.data.id}" data-id="${response.data.id}">
        <td><img src="${response.data.img}"></td>
        <td><div> ${response.data.name} </div></td>
        <td><div> ${priceFin} </div></td>
        <td><div> ${response.data.detail} </div></td>
        <td>
          <button onclick="modifyList('${response.data.id}')" id="modiBtn">수정</button>
          <button onclick="deleteList()" data-id="${response.data.id}">삭제</button>
        </td>
      </tr>`;
      ...
      Swal.fire({
        icon: "error",
        title: "업로드 실패",
        text: "파일 업로드 중 문제가 발생했습니다. 다시 시도해주세요.",
      });
    }
  });
